...
def get_real_id(id, with_suffix=False):
    ...
    if len(real_id) != 11:
        raise ValueError
    if with_suffix:
        start = train_ann.iloc[idx].time_start
        end = train_ann.iloc[idx].time_end
...
for vp in tqdm(list(train_p.glob('**/*.*'))):
    if "youtube" in vp.stem:
        ...
        real_vp_id = get_real_id(vp_id, with_suffix=True)
        ...
        if len(vp_id) == 11:
            ...
            real_vp_id = get_real_id(vp_id, with_suffix=True)
            # print(f"Renaming {vp} to {vp.with_stem(real_vp_id)}")
            vp.rename(vp.with_stem(real_vp_id))
            processed += 1
