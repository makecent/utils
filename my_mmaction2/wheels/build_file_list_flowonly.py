# This filed is a modified mmaction2/tools/data/build_file_list.py
# to support building file list when only optical flow files are available (no RGB).
# This .py can NOT be used alone but is expected to replace the original build_file_list.py.
    ...
    def build_list(split):
        ...
        for item in split:
            if item[0] not in frame_info:
                continue
            if frame_info[item[0]][2] > 0:                                      # Modified
                # rawframes
    ...
    if len(splits) > 1:
        ...
        if args.output_format == 'txt':
            with open(osp.join(out_path, filename), 'w') as f:
                f.writelines(lists[1][ind])                                     # Modified
        elif args.output_format == 'json':
